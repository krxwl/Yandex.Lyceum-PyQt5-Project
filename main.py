...
from PyQt5.QtWidgets import QApplication, QLabel, QGridLayout, QWidget, QDesktopWidget
from PyQt5.QtWidgets import QMainWindow, QTableWidgetItem, QHeaderView, QInputDialog
from PyQt5.QtGui import QColor, QCursor, QPixmap
from PyQt5 import QtCore, QtGui, QtWidgets


def center(self):  # метод для центрирования окна по центру
...
class MyWidget(QMainWindow):
    ...
    def open_book(self, ID, title, author, year, link):
        dialog = PopUp(ID, title, author, year, link)
    ...
    def pic_clicked(self, event, sender=None):
        name = self.buttons[sender]
        con = sqlite3.connect("books_db.db")
        cur = con.cursor()
